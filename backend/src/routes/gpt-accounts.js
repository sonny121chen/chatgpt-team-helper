...
router.post('/', async (req, res) => {
  try {
    ...
    // 默认有效期 30 天
    const defaultExpiresDays = 30
    const autoExpiresAt = new Date(Date.now() + defaultExpiresDays * 24 * 60 * 60 * 1000).toISOString()
    ...
    for (let i = 0; i < codesToGenerate; i++) {
      ...
      while (attempts < 5 && !success) {
        try {
          db.run(
            `INSERT INTO redemption_codes (code, account_email, expires_at, created_at, updated_at) VALUES (?, ?, ?, DATETIME('now', 'localtime'), DATETIME('now', 'localtime'))`,
            [code, normalizedEmail, autoExpiresAt]
          )
